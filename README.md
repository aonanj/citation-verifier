...
- **Verification**:
  ...
  - **Federal law**: GovInfo link service with reporter-aware URL building for U.S.C., C.F.R., Stat., Pub. L., Fed. Reg., and related materials. A U.S.C. or C.F.R. section GovInfo doesn't have (in the current or the cited year's edition) is checked against the current official source for material newer than GovInfo's latest edition: eCFR for the C.F.R. (from 2017) and the Office of the Law Revision Counsel's U.S. Code (uscode.house.gov) when that year's U.S.C. edition isn't published yet. C.F.R. sections that are reserved, or no longer in the current edition, are reported not found. A Federal Register page, or a Statutes at Large page in a published volume, that GovInfo doesn't find is reported not found. The model in `AI_MODEL`, with web search limited to Justia, Cornell LII, and FindLaw (as for state law), checks only what no official source can: subsections of a confirmed section (e.g. `35 U.S.C. § 154(a)(1)-(2)`), years before GovInfo's editions, Statutes at Large volumes not yet published, and sources GovInfo can't look up (e.g. the U.S. Constitution, a public law before the 104th Congress); its answer counts as verified only at confidence ≥ 0.90 (warning at ≥ 0.70).
  - **State law**: The model in `AI_MODEL` (an OpenAI `gpt…` model, e.g. `gpt-5.6-terra`, via the Responses API) with built-in web search tool access (Justia, Cornell LII, FindLaw) to score validity and return a matching or nearly matching citation, as well as a confidence score corresponding to verification status: verified at ≥ 0.90, warning at ≥ 0.70, otherwise not found (the model's own status can only lower that). 
